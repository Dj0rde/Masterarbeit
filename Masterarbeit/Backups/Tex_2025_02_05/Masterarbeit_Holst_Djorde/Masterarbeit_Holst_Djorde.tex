...
\begin{JavaScript}[ float,
caption = {The first Javacript code here.},
label = {label-test1},
]
Name.prototype = {
...
Javascript im Text: \JavaScript{var i} weiterer Text \JavaScript[showspaces=false]{var i}.
...
In our case, the parent node represents an action or cause, the child represents the outcome, and the arrow represents a direct causal relationship. Accordingly, a missing edge indicates a missing direct causal relationship between two nodes. 
...
\label{subsection-Confounding_Factors}
...
the outcome where $i$ did take the pill ($Y_i |_{\text{ do}(T=1)} \triangleq Y_i(1)$) 

and the outcome where $i$ did not ($Y_i |_{\text{ do}(T=0)} \triangleq Y_i(0)$).\\
...
\subsection{Granger Causality in Time Series Analysis}
...
Go through the following steps 1 by 1 and give your reasoning for each.
